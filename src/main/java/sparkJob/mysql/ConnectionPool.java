...
public class ConnectionPool implements Serializable {
    ...
    private static int initialConnections = 20; // 连接池的初始大小

    private static int incrementalConnections = 10;// 连接池自动增加的大小

    private static int maxConnections = 100; // 连接池最大的大小

    private static Vector connections = null; // 存放连接池中数据库连接的向量 , 初始时为 null 它中存放的对象为 PooledConnection 型
    ...
    private boolean testConnection(Connection conn) {
        try {
            if (testTable.equals("")) {
            // 如果测试表为空，试着使用此连接的 setAutoCommit() 方法来判断连接否可用（此方法只在部分数据库可用，如果不可用,抛出异常）。注意：使用测试表的方法更可靠
